...
from prompt_t5.t5_model.configuration_t5 import T5Config
from prompt_t5.t5_model.tokenization_t5 import *
from prompt_t5.t5_model.modeling_t5 import *
import logging

torch.manual_seed(1216)
torch.cuda.manual_seed(1216)
torch.cuda.manual_seed_all(1216)
...
def main(cfg: DictConfig) -> None:
    ...
    model_path_id = 't5-base'
    tokenizer = T5Tokenizer.from_pretrained(model_path_id)
    config = T5Config.from_pretrained(model_path_id)
    print(config)
    ...
    if cfg.mode == 'train':
        ...
        dataloader = DataLoader(dataset, shuffle=False, sampler=sampler, batch_size=batch_size, 
        ...
        for epoch in iterator:
            output_dir = "checkpoints/fid_ours/cl_one/checkpoint-epoch-{}".format(epoch)
            print(output_dir)
    ...
    if cfg.mode == 'eval':
        model.eval()
        dev_examples = load_data(os.path.join(root_folder, cfg.dataset.test_file))
        ...
        for entry in dev_examples:
            references[entry['id']] = entry['targets']
        ...
        for step, batch in enumerate(tqdm(dataloader, desc="Iteration")):
            ...
            with torch.no_grad():
                ...
                for k, o in enumerate(outputs):
                    ...
                    assert example['id'] not in results
                    results[example['id']] = ans

        scores = get_raw_scores(results, references)
        print('evaluation results', scores)
        with open('output/examples_scores/fid-1-ours-scores.json', 'w') as f1:
           json.dump(scores,f1)

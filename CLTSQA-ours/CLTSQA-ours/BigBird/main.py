...
class TSQAExample(object):
    def __init__(
        ...
        self.contrast_question_text = contrast_question_text
        self.similar_question_text = similar_question_text
...
class TSQAProcessor(DataProcessor):
    ...
    def _create_examples(self, is_training: bool):
        ...
        for entry in tqdm(self.dataset):         
            ...
            contrast_question_text = entry["contrast_question"]
            simliar_question_text = entry["similar_question"]
            if is_training:
                for target, start_position in zip(entry['targets'], entry['from']):
                    example = TSQAExample(
                        qas_id=qas_id,
                        question_text=question_text,
                        contrast_question_text=contrast_question_text,
                        similar_question_text=simliar_question_text,
    ...
    def _get_reference(self):
        ...
        for entry in self.dataset:
            references[int(entry['id'])] = entry['targets']
        return references
...
class TSQAFeatures(object):
    def __init__(
        ...
        self.contrast_input_ids = contrast_input_ids
        self.similar_input_ids = similar_input_ids
        self.attention_mask = attention_mask
        self.contrast_attention_mask = contrast_attention_mask
        self.similar_attention_mask = similar_attention_mask
...
def convert_examples_to_features(examples, tokenizer, max_seq_length, doc_stride, max_query_length, is_training):
    ...
    all_contrast_input_ids = torch.tensor([f.contrast_input_ids for f in features], dtype=torch.long) 
    all_similar_input_ids = torch.tensor([f.similar_input_ids for f in features], dtype=torch.long)
    all_attention_masks = torch.tensor([f.attention_mask for f in features], dtype=torch.long)
    all_contrast_attention_masks = torch.tensor([f.contrast_attention_mask for f in features],dtype=torch.long)
    all_similar_attention_masks = torch.tensor([f.similar_attention_mask for f in features], dtype=torch.long)
    ...
    if not is_training:
        ...
        dataset = TensorDataset(
            all_input_ids,
            all_contrast_input_ids,
            all_similar_input_ids,
            all_attention_masks,
            all_contrast_attention_masks,
            all_similar_attention_masks,
            all_start_positions,
            all_end_positions,
            all_is_impossible,
        )
...
def convert_example_to_features(example, max_seq_length, doc_stride, max_query_length, is_training):
    ...
    truncated_contrast_query = tokenizer.encode(example.contrast_question_text, add_special_tokens=False,
            max_length=max_query_length, truncation=True)
   
    truncated_similar_query = tokenizer.encode(example.similar_question_text, add_special_tokens=False,
            max_length=max_query_length, truncation=True)
    ...
    while len(spans) * doc_stride < len(all_doc_tokens):
        ...
        encoded_contrast_dict = tokenizer.encode_plus(
            truncated_contrast_query,
            span_doc_tokens,
            max_length=max_seq_length,
            return_overflowing_tokens=True,
            padding='max_length',
        ...
        encoded_similar_dict = tokenizer.encode_plus(
            truncated_similar_query,
            span_doc_tokens,
            max_length=max_seq_length,
            return_overflowing_tokens=True,
            padding='max_length',
        ...
        if tokenizer.pad_token_id in encoded_contrast_dict["input_ids"]:
            non_padded_ids = encoded_contrast_dict["input_ids"][: encoded_contrast_dict["input_ids"].index(tokenizer.pad_token_id)] 
        else:
            non_padded_ids = encoded_contrast_dict["input_ids"] 

        if tokenizer.pad_token_id in encoded_similar_dict["input_ids"]: 
             non_padded_ids = encoded_similar_dict["input_ids"][: encoded_similar_dict["input_ids"].index(tokenizer.pad_token_id)]
        ...
        contrast_spans.append(encoded_contrast_dict)
        similar_spans.append(encoded_similar_dict)
    ...
    for i in range(len(spans)):
        ...
        cls_index = spans[i]["input_ids"].index(tokenizer.cls_token_id)
        assert cls_index == 0, cls_index

        span_is_impossible = example.is_impossible
        
        # Setting start/end position to the last + 1 position to be ignored
        start_position = len(spans[i]['input_ids'])
        end_position = len(spans[i]['input_ids'])
        
        if is_training and not span_is_impossible:
            # For training, if our document chunk does not contain an annotation
            # we throw it out, since there is nothing to predict.
            doc_start = spans[i]["start"]
            doc_end = spans[i]["start"] + spans[i]["length"] - 1
            out_of_span = False
        ...
        features.append(
            TSQAFeatures(
                input_ids=spans[i]["input_ids"],
                contrast_input_ids=contrast_spans[i]["input_ids"],
                similar_input_ids=similar_spans[i]["input_ids"],
                attention_mask=spans[i]["attention_mask"],
                contrast_attention_mask=contrast_spans[i]["attention_mask"],
                similar_attention_mask=similar_spans[i]["attention_mask"],
                cls_index=cls_index,
                qas_id=example.qas_id,  # Can not set unique_id and example_index here. They will be set after multiple processing.
                paragraph_len=spans[i]["paragraph_len"],
                token_is_max_context=spans[i]["token_is_max_context"],
                tokens=spans[i]["tokens"],
                token_to_orig_map=spans[i]["token_to_orig_map"],
...
class BigBirdForQuestionAnsweringWithNull(PreTrainedModel):
    def __init__(self, config, model_id):
        ...
        self.ranking_loss = nn.TripletMarginLoss(margin=1.0, p=2.0, eps=1e-06, swap=False, size_average=None, reduce=None, reduction='mean')
    ...
    def forward(self, **kwargs):
        if self.training:
            ...
            similar_pooler_output = outputs.similar_pooler_output
            contrast_pooler_output = outputs.contrast_pooler_output
            null_logits = self.null_classifier(pooler_output)
            similar_null_logits = self.null_classifier(similar_pooler_output)
            loss_fct = CrossEntropyLoss()
            null_loss = loss_fct(null_logits, null_labels)
            similar_null_loss = loss_fct(similar_null_logits, null_labels)

            outputs.loss = outputs.loss + null_loss
            outputs.similar_loss = outputs.similar_loss + similar_null_loss
            contrast_loss = self.ranking_loss(pooler_output.view(-1, pooler_output.size(-1)), similar_pooler_output.view(-1, similar_pooler_output.size(-1)), contrast_pooler_output.view(-1, contrast_pooler_output.size(-1)))

            return outputs.to_tuple(), contrast_loss
        else:
            outputs = self.bertqa(**kwargs)
            pooler_output = outputs.pooler_output
            null_logits = self.null_classifier(pooler_output)
...
def main(cfg: DictConfig) -> None:
    ...
    if cfg.mode == 'train':
        ...
        for epoch in iterator:
            output_dir = "checkpoints/bigbird_ours/cl_one/checkpoint-epoch-{}".format(epoch)
            ...
            for step, batch in enumerate(tqdm(dataloader, desc="Iteration")):
                ...
                inputs = {
                    "input_ids": batch[0],
                    "contrast_input_ids": batch[1],
                    "similar_input_ids": batch[2],
                    "attention_mask": batch[3],
                    "contrast_attention_mask": batch[4],
                    "similar_attention_mask":batch[5],
                    "start_positions": batch[6],
                    "end_positions": batch[7],
                    "is_impossible": batch[8]
                }
                outputs, contrast_loss = model(**inputs)
                print("loss1: " + str(outputs[0]) + " loss2: " + str(outputs[1]) + " loss3: " + str(contrast_loss))
                
                loss = outputs[0] + 0.5*outputs[1] + 0.5*contrast_loss
                if cfg.n_gpu > 1:
                    loss = loss.mean()
